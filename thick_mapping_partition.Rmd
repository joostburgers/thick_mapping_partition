...
The distribution pattern of locations mentioned shows that men without occupations make a negligible impact on the mean number of locations mentioned. Meanwhile, this is what tends to skew the women downwards more. It is not as if men overwhelming mention more locations, but a few men mention quite a few more locations.

```{r histogram_mean_locations}
 p <- partition_statistics %>% 
  ...
  p %>% ggplot(aes(loc_by_name, fill = gender))+
    geom_histogram(color="gray", opacity=.5,alpha=.4, position = "identity" )+
