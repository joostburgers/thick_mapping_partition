...
The script relies on the `tidygeocoder` package developed by Jesse Cambon, Diego Hernangómez, Christopher Belanger, Daniel Possenriede: (https://jessecambon.github.io/tidygeocoder/index.html) The package allows users to select the geocoder of their choice. For the purposes of easy reproduceability OpenStreetMap (`osm`) was selected, though other services that require registration or login might be more accurate. This process is time consuming and has been commented out. The address file has been cached.
...
* *_known* - Whether the location was ultimately found. FALSE indication that the location is a best guess

* *_camp* - Indicates that this was a refugee camp. This data was not used
...
write_csv(partition_geolocations, "data/partition_geolocations_raw.csv")
```
...
partition_clean <- read_csv("data/partition_geolocations_clean.csv", na="NA")
```
